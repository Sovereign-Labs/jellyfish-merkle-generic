...
use super::{skip_common_prefix, NibblePath};
use crate::{Nibble, TEST_DIGEST_SIZE};
use proptest::prelude::*;

#[test]
fn test_nibble_path_fmt() {
    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![0x12, 0x34, 0x56]);
    assert_eq!(format!("{nibble_path:?}"), "123456");

    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![0x12, 0x34, 0x50]);
    assert_eq!(format!("{nibble_path:?}"), "123450");

    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_odd(vec![0x12, 0x34, 0x50]);
...
fn test_create_nibble_path_success() {
    ...
    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![]);
...
fn test_create_nibble_path_failure() {
    ...
    let _nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_odd(bytes);
}

#[test]
#[should_panic(expected = "Should have odd number of nibbles.")]
fn test_empty_nibble_path() {
    NibblePath::<TEST_DIGEST_SIZE>::new_odd(vec![]);
}

#[test]
fn test_get_nibble() {
    let bytes = vec![0x12, 0x34];
    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_even(bytes);
...
fn test_get_nibble_from_byte_array() {
    ...
    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_from_byte_array(bytes.as_slice(), 4);
    ...
    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_from_byte_array(bytes.as_slice(), 3);
...
fn test_nibble_iterator() {
    ...
    let nibble_path = NibblePath::<TEST_DIGEST_SIZE>::new_odd(bytes);
...
fn test_skip_common_prefix() {
    {
        let nibble_path1 = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![0x12, 0x34, 0x56]);
        let nibble_path2 = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![0x12, 0x34, 0x56]);
    ...
    {
        let nibble_path1 = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![0x12, 0x35]);
        let nibble_path2 = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![0x12, 0x34, 0x56]);
        let mut iter1 = nibble_path1.nibbles();
        let mut iter2 = nibble_path2.nibbles();
    ...
    {
        let nibble_path1 = NibblePath::<TEST_DIGEST_SIZE>::new_even(vec![0x12, 0x34, 0x56]);
        let nibble_path2 = NibblePath::<TEST_DIGEST_SIZE>::new_odd(vec![0x12, 0x30]);
        let mut iter1 = nibble_path1.nibbles();
        let mut iter2 = nibble_path2.nibbles();
        assert_eq!(skip_common_prefix(&mut iter1, &mut iter2), 3);
        assert_eq!(
...
prop_compose! {
    fn arb_nibble_path_and_current()(nibble_path in any::<NibblePath<TEST_DIGEST_SIZE>>())
        (current in 0..=nibble_path.num_nibbles(),
         nibble_path in Just(nibble_path)) -> (usize, NibblePath<TEST_DIGEST_SIZE>) {
        (current, nibble_path)
    }
}
...
proptest! {
    #[test]
    fn test_push(
        nibble_path in any::<NibblePath<TEST_DIGEST_SIZE>>().prop_filter( "Filter out leaf paths.",
        |p| p.num_nibbles() < NibblePath::<TEST_DIGEST_SIZE>::ROOT_NIBBLE_HEIGHT),
    ...
    #[test]
    fn test_pop(mut nibble_path in any::<NibblePath<TEST_DIGEST_SIZE>>()) {
        let mut nibbles: Vec<Nibble> = nibble_path.nibbles().collect();
    ...
    #[test]
    fn test_last(mut nibble_path in any::<NibblePath<TEST_DIGEST_SIZE>>()) {
    ...
    #[test]
    fn test_nibble_iter_roundtrip(nibble_path in any::<NibblePath<TEST_DIGEST_SIZE>>()) {
