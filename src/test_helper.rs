...
#[derive(
    Arbitrary, Clone, Debug, Default, PartialOrd, Ord, Eq, Hash, PartialEq, Serialize, Deserialize,
)]
pub struct ValueBlob(Vec<u8>);

impl AsRef<[u8]> for ValueBlob {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}
...
impl<'a> From<&'a [u8]> for ValueBlob {
...
impl crate::Key for ValueBlob {
    ...
    type FromBytesErr = Infallible;
}
impl TestKey for ValueBlob {}
    ...
    TestHash: TreeHash<N>,
{
    ...
    let tree = JellyfishMerkleTree::<_, V, TestHash, N>::new(&db);
    ...
    TestHash: TreeHash<N>,
{
    ...
    for key in nonexistent_keys {
        let (account, proof) = tree.get_with_proof(*key, version).unwrap();
        let res = proof.verify_by_hash(root_hash, *key, None);
        if res.is_err() {
            panic!("expected result to be ok but got err: {:?}", res)
        }
    ...
    TestHash: TreeHash<N>,
{
    ...
    for (key, value) in &btree {
        let leaf =
            LeafNode::<_, TestHash, N>::new(*key, value.0, (value.1.clone(), 0 /* version */));
    ...
    TestHash: TreeHash<N>,
{
    ...
    compute_root_hash_impl::<TestHash, N>(kv_ref)
}
    ...
    TestHash: TreeHash<N>,
{
    ...
    match kvs.iter().position(|(key, _value)| key[0]) {
        Some(0) => {
            ...
            left_hash = H::SPARSE_MERKLE_PLACEHOLDER_HASH;
            right_hash = compute_root_hash_impl::<TestHash, N>(reduce(&kvs));
        }
        Some(index) => {
            // Both left subtree and right subtree have some keys.
            left_hash = compute_root_hash_impl::<TestHash, N>(reduce(&kvs[..index]));
            right_hash = compute_root_hash_impl::<TestHash, N>(reduce(&kvs[index..]));
        }
        None => {
            // Every key starts with a 0-bit, i.e., they are all in the left subtree.
            left_hash = compute_root_hash_impl::<TestHash, N>(reduce(&kvs));
            right_hash = H::SPARSE_MERKLE_PLACEHOLDER_HASH;
        }
    }
